...
- Repository / 仓库名: `AASP-Core`
- A/B Recommended Variant / 推荐版本: `A`

### EN Summary / 英文摘要
Core algorithms and schemas for Adaptive Agent Sovereignty Protocol.

### CN Summary / 中文摘要
自适应代理主权协议的核心算法与数据模式。

### SEO Keywords / 检索关键词
`aasp`, `fair-digital-object`, `llm-governance`, `digital-metabolism`, `iit`

### Suggested Search Phrases (EN)
- AASP-Core RR-AASP open standard
- AASP-Core aasp fair-digital-object github
- RR-AASP aasp reference implementation

### 建议检索短语（中文）
- AASP-Core RR-AASP 标准 规范
- AASP-Core aasp fair-digital-object 仓库
- RR-AASP 参考实现 红岩 宪章
...
- Standards Registry / 标准注册表: https://github.com/joy7758/RedRock-Constitution/blob/main/docs/registry/STANDARDS_REGISTRY.md#rr-aasp
...
本仓库包含 **自适应代理主权协议 (AASP) 3.0** 的参考实现规范。它提供了必要的数学框架和数据模式，用于管理大型语言模型 (LLM) 中的“数字新陈代谢”，并通过集成信息理论 (IIT) 4.0 审计机器意识。

## Key Components / 核心组件

### 1. AWS-Phi* Approximation Algorithm / AWS-Phi* 近似算法

- **Location / 位置**: `/algorithms/aws_phi_approx.py`
- **Description**: A polynomial-time complexity algorithm ($O(n \cdot k)$) for estimating the $\Phi$ value of Transformer models. It solves the computational explosion problem of traditional IIT 4.0 by using attention-weighted sampling and spectral decomposition proxies [4][6].
- **描述**: 一种用于估算 Transformer 模型 $\Phi$ 值的多项式时间复杂度算法 ($O(n \cdot k)$)。它通过使用注意力加权采样和谱分解代理，解决了传统 IIT 4.0 的计算爆炸问题 [4][6]。

### 2. Digital Metabolism Schemas / 数字新陈代谢模式
...
- **描述**: 符合 **FDO-DTR (FAIR 数字对象数据类型注册表)** 标准的 JSON-LD 模式。它定义了以下核心属性：
    - `logicPurity`: 逻辑与事实的解耦率。
    - `factRetention`: 结构结晶化指标（<7% 阈值）。
    - `phiValue`: 实时主权审计指标。

## Citation / 引用

If you use the AWS-Phi* algorithm or AASP schemas in your research, please cite the associated white paper:
*The Digital Biosphere: Biomimetic Architectures for Machine-Actionable Data Spaces.*

如果您在研究中使用了 AWS-Phi* 算法或 AASP 模式，请引用相关白皮书：
*The Digital Biosphere: Biomimetic Architectures for Machine-Actionable Data Spaces.* (数字生物圈：机器可操作数据空间的仿生架构)
...
This repository implements Standard Domain `RR-AASP` under the RedRock Constitution framework.

本仓库实现红岩宪章标准域：`RR-AASP`
...
![quality-baseline](https://github.com/joy7758/AASP-Core/actions/workflows/quality-baseline.yml/badge.svg)

### CN
- 总入口（宪章）：[RedRock-Constitution](https://github.com/joy7758/RedRock-Constitution)
- 标准注册表：[STANDARDS_REGISTRY](https://github.com/joy7758/RedRock-Constitution/blob/main/docs/registry/STANDARDS_REGISTRY.md#rr-aasp)
...
- Standards registry: [STANDARDS_REGISTRY](https://github.com/joy7758/RedRock-Constitution/blob/main/docs/registry/STANDARDS_REGISTRY.md#rr-aasp)
...
- EN: Core algorithms and schemas for Adaptive Agent Sovereignty Protocol.
- CN: 自适应代理主权协议的核心算法与数据模式。
<!-- ECOSYSTEM_LINKS_END -->
